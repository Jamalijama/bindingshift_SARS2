...
for file in os.listdir(path):
    if file.endswith('.csv') & file.startswith('df_spikeprot_S1dedup_S1high95_Shigh95_132204_v2_sampled100000'):
        df = pd.read_csv(path + file, index_col='seqID', encoding='gbk')

sequences = df.prS1.tolist()
...
training_args = TrainingArguments(
    output_dir="./esm2_finetuned_SARS2S1",  # 模型保存路径
...
# 7. 保存微调后的模型和分词器
model.save_pretrained("./esm2_finetuned_final")
tokenizer.save_pretrained("./esm2_finetuned_final")
