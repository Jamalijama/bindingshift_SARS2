...
import os
import pickle
from data_load_SARS2S1_SARSMERSesm2 import loading_data
...
class Evaluator:
    ...
    def plot_roc_curve(self, probs, labels, save_path='roc_curve.svg'):
...
def main_training_workflow(X, y, \
    ...
    # Evaluate model
    evaluator = Evaluator(trained_model, device)
    ...
    # Plot ROC curve
    evaluator.plot_roc_curve(probs, labels, save_path = 'roc_curve_' + model_name[:4] + '.svg')
    ...
    plt.tight_layout()
    plt.savefig('training_results' + model_name +'.svg', dpi=600, bbox_inches='tight')
    plt.show()
