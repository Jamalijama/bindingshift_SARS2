...
# path_model = '../embedding_SpikeS1_SARSMERS_ESM2finetuned/'
...
label_1 = df_labels['label'].tolist()
print(len(label_1))
seq_idlst0 = df_labels['Accession ID'].tolist()
 
path_model = './'
model_lst0 = []


for i in range(100, 8300,200):
    # print (i)
    # model_lst.append()
    file = 'checkpoint-' + str(i) + '_spikeprot_S1dedup_Shigh95_132204_v2_testing32204_SARSMERSesm2_embedded.pkl'
    model_lst0.append(path_model + file)

for model in model_lst0[2:]:
    print (model)    
    ...
    with open (model, 'rb') as f:
        ...
        df_array0 = pd.DataFrame (data = reshape_composition_array, index = seq_idlst0)
        df_array0['label'] = label_1
        # df_array0['seqID'] = seq_idlst
        # df_array0 = df_array0[df_array0['label'].isin([0, 1, 2])]     
    
        # file_name = 'esm2_t33_650M_UR50D_spikeprot_S1dedup'
        file_name = model.split('/')[-1][:-4]
        print (file_name)
        ...
        for rand_i in range(0,10,1):
            ...
            df_label_0 = df_label0[df_label0['label'] == 0].sample(n = 1000, random_state = rand_i)
            df_label_1 = df_label0[df_label0['label'] == 1].sample(n = 1000, random_state = rand_i)
            df_label_2 = df_label0[df_label0['label'] == 2].sample(n = 1000, random_state = rand_i)
            
            
            df_label = pd.concat([df_label_0, df_label_1, df_label_2]) 
            ...
            label_1_sampled = df_array.iloc[:,-1].tolist()
        
            y_types = list(set(label_1_sampled))
            y_num = len(y_types)
            ...
            ac_cluster_1_pred = MiniBatchKMeans(n_clusters=k_selected, random_state=10).fit_predict(reshape_composition_array)
        
            # ac_cluster_1_pred = AgglomerativeClustering(n_clusters=2, linkage='complete').fit_predict(X_pca)
            # # 聚类效果评价
            # print('\nagglomerative clustering:')
            # # accuracy，值越大越好
            acc_1 = accuracy_score(label_1_sampled, ac_cluster_1_pred)
            ...
            # # 调整兰德指数，值越大越好
            ari_score_1 = adjusted_rand_score(label_1_sampled, ac_cluster_1_pred)
            print('ari_score =', ari_score_1)
            # # 标准化互信息，值越大越好
            nmi_score_1 = normalized_mutual_info_score(label_1_sampled, ac_cluster_1_pred)
    ...
    df_evaluation.to_csv('./df_evaluate_cluster_' + file_name + '_withPCA_label.csv')
